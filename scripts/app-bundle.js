...
define('basicinputs/combobox',["require", "exports", "@fluentui/react/lib/ComboBox"], function (require, exports, ComboBox_1) {
    ...
    var combobox = (function () {
        function combobox() {
            this.testOptions = [
                { key: 'Header', text: 'Theme Fonts', itemType: ComboBox_1.SelectableOptionMenuItemType.Header },
                { key: 'A', text: 'Arial Black' },
                { key: 'B', text: 'Times New Roman' },
                { key: 'C', text: 'Comic Sans MS', selected: true },
                { key: 'divider_2', text: '-', itemType: ComboBox_1.SelectableOptionMenuItemType.Divider },
                { key: 'Header1', text: 'Other Options', itemType: ComboBox_1.SelectableOptionMenuItemType.Header },
                { key: 'D', text: 'Option d' },
...
;
define('text!basicinputs/combobox.html',[],function(){return "<template><div class=content style=bottom:unset;top:0;background-color:#038387><h1 class=pageTitle>Combobox</h1><div role=presentation class=ms-FocusZone></div></div><div class=PageSection><examplecard title=ChoiceGroups><div slot=example><du-combo-box defaultselectedkey=C label=\"Basic uncontrolled example (allowFreeform: true, AutoComplete: on):\" allow-freeform=true auto-complete=on options.one-time=testOptions></du-combo-box><du-combo-box multi-select=true default-selected-key.one-time=\"['C', 'E']\" label=\"Basic uncontrolled example (allowFreeform: true, AutoComplete: on):\" allow-freeform=true auto-complete=on options.one-time=testOptions></du-combo-box><du-combo-box multi-select=true default-selected-key.one-time=\"['C', 'E']\" label=\"Basic controlled example (allowFreeform: true, AutoComplete: on):\" allow-freeform=true auto-complete=on options.bind=testOptions on-menu-open.bind=menuOpen on-menu-dismissed.bind=dismissed on-focus.bind=onFocus on-blur.bind=onBlur on-change.bind=onChange></du-combo-box></div></examplecard></div></template>";});;
...
define('main',["require", "exports", "aurelia-framework", "./environment"], function (require, exports, aurelia_framework_1, environment_1) {
    ...
    function configure(aurelia) {
        aurelia.use
            ...
            aurelia_framework_1.PLATFORM.moduleName('./resources/elements/au-hljs')
        ]);
...
;
define('text!reqjs.html',[],function(){return "<template><div class=content style=bottom:unset;top:0;background-color:#038387><h1 class=pageTitle>Getting started</h1><div role=presentation class=ms-FocusZone></div></div><div class=PageSection><h2 class=ms-font-xxl>Get started with Office UI Fabric for Aurelia</h2><ol class=steps><li><p>Create your project the way you are used to, then follow below instructions if you are using RequireJS.</p></li><li><p>Add the npm package @dunite/au-office-ui with below command. We have tested with npm and yarn, if you are using anything else please give feedback if it does not work for you.</p><p>If you have issues with building refering to \"text\", then remove the developer dependency to requirejs/text and add that again with below command</p><au-hljs language=shell>npm install @dunite/au-office-ui --save npm install babel-eslint eslint @types/react @types/react-dom --save-dev npm install requirejs/text --save-dev</au-hljs></li><li><p>Add a file called nodeprocess.js at the root of your project. Same level as the node_modules folder.</p><p>Insert below content:</p><au-hljs language=javascript>var process = {} || process; process.browser = true; process.env = { NODE_ENV : 'production' }</au-hljs><p>Then add below to your aurelia.json file. It is just the line refering to nodeprocess.js you need to add.</p><au-hljs language=json>\"name\": \"vendor-bundle.js\", \"prepend\": [ \"nodeprocess.js\", \"node_modules/requirejs/require.js\" ],</au-hljs></li><li><p>In your main.ts file add the following to register the plugin. Line 7 is required.</p><au-hljs language=javascript linenumbers=true>export function configure(aurelia: Aurelia) { aurelia.use .standardConfiguration() .feature(PLATFORM.moduleName('resources/index')) // below is the new line you need .plugin(PLATFORM.moduleName(\"@dunite/au-office-ui\")); aurelia.use.developmentLogging(environment.debug ? 'debug' : 'warn'); if (environment.testing) { aurelia.use.plugin(PLATFORM.moduleName('aurelia-testing')); } return aurelia.start().then(() => aurelia.setRoot(PLATFORM.moduleName('app'))); }</au-hljs></li><li><p>As of version 1.2.0 no elements are registred by the plugin. That means you need to require them with the &lt;require&gt;&lt;/require&gt; or preload them with a call to globalResources(). This will bring down bundle sizes. Below line will register one component and then it will be ready to use on any page.</p><au-hljs language=javascript>aurelia.use .standardConfiguration() .plugin(PLATFORM.moduleName(\"@dunite/au-office-ui\")) .globalResources( [ // Registrera alla komponenter som denna webpart använder här PLATFORM.moduleName('@dunite/au-office-ui/resources/elements/BasicInputs/DuActionButton') ]);</au-hljs><p>Check out all elements in the navigation to the left and read the specific documentation for each element.</p><p>All elements use du- prefix. That means that the colorpicker element is used with the html below.</p><au-hljs language=html>&#x3C;du-color-picker alpha-slider-hidden=&#x22;true&#x22; color.bind=&#x22;thecolor&#x22; on-color-changed.bind=&#x22;colorChanged&#x22; &#x3E;&#x3C;/du-color-picker&#x3E;</au-hljs><p>As always we use kebab-casing for the elements and the properties. If you read the documentation on the React Office UI Fabric site about the property onColorChanged it is used with on-color-changed.</p></li><li><p>Regarding binding, it is Aurelia easy. In most cases you can set the value of the property directly in HTML like the alpha-slider-hidden property above. Sometimes though the React component does not parse the value correctly so you have to add .bind even if it is a simple number or boolean. It never seems to be an issue with strings though.</p><p>It is also possible to bind to literal objects as showed below with icon-props.</p><au-hljs language=html linenumbers=true>&#x3C;du-command-bar-button text=&#x22;Create Account&#x22; on-click.bind=&#x22;actionButtonClick&#x22; icon-props.one-time=&#x22;{ iconName: &#x27;Add&#x27; }&#x22; menu-props.bind=&#x22;commandMenu&#x22; split.bind=&#x22;true&#x22;&#x3E;&#x3C;/du-command-bar-button&#x3E;</au-hljs><p>Note that you need to use .one-time since Aurelia can not update the literal object.</p><p>If you add bindings to any event always use .bind with the function name, see on-click.bind example above.</p><p>As of version 2.0.0 of au-office-ui the 'this' keyword always points the current view model as you would expect. This is a breaking change from previous version where the 'this' keyword refered to the au-component.</p></li></ol></div></template>";});;
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
...
;
define('spfx',["require", "exports"], function (require, exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.spfx = void 0;
    var spfx = (function () {
...
;
define('text!spfx.html',[],function(){return "<template><require from=./gettingstarted.css></require><div class=content style=bottom:unset;top:0;background-color:#038387><h1 class=pageTitle>Getting started with SPFx</h1><div role=presentation class=ms-FocusZone></div></div><div class=PageSection><h2 class=ms-font-xxl>Get started with Office UI Fabric for Aurelia on SPFx</h2><p></p><ol class=steps><li><p>Follow the instructions at https://docs.microsoft.com/en-us/sharepoint/dev/spfx/set-up-your-development-environment to set up your development environemnt.</p><p>Verify that you can run your webpart or extension with gulp serve.</p></li><li><p>Add the npm packages with below command.</p><au-hljs language=shell>npm install @dunite/au-office-ui --save npm install aurelia-bootstrapper aurelia-fetch-client bluebird@3.5.0 --save npm install aurelia-webpack-plugin@2.0.0-rc.5 --save-dev</au-hljs></li><li><p>In your webpart.ts file add the following to import necessery files.</p><au-hljs language=javascript linenumbers=true>import * as Bluebird from 'bluebird'; import {bootstrap} from 'aurelia-bootstrapper'; import { Aurelia,TemplatingEngine } from 'aurelia-framework'; import { PLATFORM } from \"aurelia-pal\";</au-hljs></li><li><p>Then we add a constructor to configure bluebird to handle Promise for us.</p><au-hljs language=javascript>constructor() { super(); Bluebird.config({ warnings: { wForgottenReturn: false } }); }</au-hljs><p>Replace the render function with below code.</p><au-hljs language=javascript>public render(): void { this.domElement.innerHTML = `<div id=\"${this.instanceId}\" class=\"${this.instanceId}\"><helloworld>Loading...</helloworld></div>`; bootstrap(aurelia => { aurelia.use .standardConfiguration() .plugin(PLATFORM.moduleName('@dunite/au-office-ui')) .developmentLogging() .globalResources( [ // Registrera alla komponenter som denna webpart använder här PLATFORM.moduleName('@dunite/au-office-ui/resources/elements/ProgressValidation/DuMessageBar'), PLATFORM.moduleName('@dunite/au-office-ui/resources/elements/BasicInputs/DuDefaultButton'), PLATFORM.moduleName('@dunite/au-office-ui/resources/elements/BasicInputs/DuTextField'), PLATFORM.moduleName('@dunite/au-office-ui/resources/elements/Pickers/DuDatePicker'), PLATFORM.moduleName('@dunite/au-office-ui/resources/elements/BasicInputs/DuIconButton'), PLATFORM.moduleName('@dunite/au-office-ui/resources/elements/BasicInputs/DuCommandBarButton'), PLATFORM.moduleName('@dunite/au-office-ui/resources/elements/BasicInputs/DuCheckBox') ]); var el = document.getElementById(this.instanceId); aurelia.start().then(a => { aurelia.container.registerInstance(\"WebPartContext\", this.context); let templatingEngine = a.container.get(TemplatingEngine); templatingEngine.enhance({ container: a.container, element: el, resources: a.resources }); }); } ); }</au-hljs><p>Add a hellowworld.ts file next to your webpart.</p><au-hljs language=javascript linenumbers=true>export class helloworld { private onFormatDate(date: Date) { var mm = date.getMonth() + 1; var dd = date.getDate(); return [date.getFullYear(),'-', (mm>9 ? '' : '0') + mm,'-', (dd>9 ? '' : '0') + dd ].join(''); } }</au-hljs><p>Add a helloworld.html file next to your webpart</p><au-hljs language=html linenumbers=true>&lt;template&gt; &lt;du-date-picker format-date.bind=&quot;onFormatDate&quot; placeholder=&quot;V&#228;lj ett datum&quot; show-week-numbers=&quot;true&quot; &gt;&lt;/du-date-picker&gt; &lt;/template&gt;</au-hljs></li><li><p>We also need to update gulp.js file to use the Aurelia Webpack plugin. Add below code just before the call to build.initialize(gulp);</p><au-hljs language=javascript linenumbers=true>const { AureliaPlugin } = require(\"aurelia-webpack-plugin\"); build.configureWebpack.mergeConfig({ additionalConfiguration: (generatedConfiguration) => { //generatedConfiguration.resolve.modules.push(\"src\"); generatedConfiguration.module.rules[0].issuer = { // only when the issuer is a .js/.ts file, so the loaders are not applied inside templates test: /\\.[tj]s$/i, }; var rule1 = { test: /\\.css$/i,issuer: [{ test: /\\.html$/i }], use: \"css-loader\"} ; generatedConfiguration.module.rules.push(rule1) var rule2 = { test: /\\.ts$/i, use: \"ts-loader\" }; generatedConfiguration.module.rules.push(rule2); generatedConfiguration.plugins.push(new AureliaPlugin( { aureliaApp: undefined })); return generatedConfiguration; } });</au-hljs><p>Note that you need to use .one-time since Aurelia can not update the literal object.</p><p>If you add bindings to any event always use .bind with the function name, see on-click.bind example above.</p><p>In the event function in your view, the 'this' keyword always points to the Office-UI element. In the code example below, we use a TypeScript feature to explicitly tell the compiler which type this has. If you need to reach out to the parent element, just use this.parent. The parent property is assigned on all au-office-ui elements and points to the hosting element, the parent. The parent property is actually assigned to the binding context.</p><au-hljs language=javascript linenumbers=true>export class helloworld { }</au-hljs></li></ol></div></template>";});;
...
;
define('text!why.html',[],function(){return "<template><div class=content style=bottom:unset;top:0;background-color:#038387><h1 class=pageTitle>Why</h1><div role=presentation class=ms-FocusZone></div></div><div class=PageSection><h2 class=ms-font-xxl>Why use Office UI Fabric for Aurelia</h2><p>Every application needs a consistent UI, so does yours. There are several alternatives out there for your Aurelia application. The great about Office UI Fabric is that is what Microsoft use for Office 365, Visual Studio Online and basically all their online prescence. That means that is does not matter if you are building add-ins for Office 365 or your next great award winning public application, Office UI Fabric gives you a consistent look and feel.</p><p>The problem is that Office UI is built with React. This npm package wraps many of the React components and expose them as easy to use Aurelia components.</p><p>If you ever needed a color picker, below is all the code you need.</p><pre><code class=html>\n      &#x3C;du-color-picker alpha-slider-hidden=&#x22;true&#x22; color.bind=&#x22;thecolor&#x22;\n      on-color-changed.bind=&#x22;colorChanged&#x22; &#x3E;&#x3C;/du-color-picker&#x3E;\n    </code></pre><p>The npm package&nbsp;<a href=https://www.npmjs.com/package/@dunite/au-office-ui>@dunite/au-office-ui</a>&nbsp;is built by&nbsp;<a href=https://twitter.com/MagnusDanielson>https://twitter.com/MagnusDanielson</a>&nbsp;at&nbsp;<a href=https://www.dunite.se>https://www.dunite.se</a></p></div></template>";});;
